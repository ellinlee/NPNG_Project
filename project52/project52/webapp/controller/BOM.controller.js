sap.ui.define([
    ...
    return Controller.extend("project52.controller.BOM", {
        ...
        _onRouteMatched(oEvent) {
            ...
            if (aItems.length > 0 && aItems[0].isA("sap.m.Text") && aItems[1].isA("sap.m.Text")) {
                oVBox.removeItem(aItems[0]);
                oVBox.removeItem(aItems[1]);
            }

            // 새로운 텍스트 컨트롤 추가
            oVBox.insertItem(new Text({ text: `Material ID: ${this.headerMat.matId}` }), 0);
            oVBox.insertItem(new Text({ text: `Material Name: ${this.headerMat.matNm}` }), 1);
        },
        ...
        onValueHelpRequest(oEvent) {
            const oInput = oEvent.getSource(); // 서치 헬프가 호출된 Input 컨트롤
            const oModel = this.getOwnerComponent().getModel("bom"); // BOM 모델 가져오기

            // OData 서비스 호출
            oModel.read("/zdct_mm010Set", {
                success: (oData) => {
                    const aItems = oData.results; // OData 결과 가져오기
                    console.log("OData Results:", aItems); // 여기에서 호출해야 함
